...
TAG="v${VERSION}"
ZIP="Legday-macOS-${VERSION}.zip"
RELEASE_DIR="$ROOT/release"
ARCHIVE="$RELEASE_DIR/$ZIP"
APP_BUNDLE="build/Build/Products/Release/Legday.app"

echo "Версия: $VERSION (тег $TAG)"

# 1. Сборка
echo "Сборка Release..."
xcodebuild -scheme Legday -configuration Release -derivedDataPath build clean build -quiet

# 2. Упаковать только .app
echo "Упаковка $ZIP..."
rm -rf "$RELEASE_DIR/Legday.app" "$RELEASE_DIR/Legday-macOS-"*.zip
mkdir -p "$RELEASE_DIR"
cp -R "$ROOT/$APP_BUNDLE" "$RELEASE_DIR/"
cd "$RELEASE_DIR"
zip -rq "$ZIP" Legday.app
cd "$ROOT"
  ...
  if [ -z "$RELEASE_ID" ] || [ "$HTTP_CODE" = "404" ]; then
    ...
    RELEASE_RESP=$(curl -s -w "\n%{http_code}" -X POST \
      ...
      -d "{
        ...
        \"body\": \"Скачайте **${ZIP}** (в Assets). В архиве один файл — Legday.app. Перетащите в «Программы». Не скачивайте Source code — там исходники.\"
      }")
    HTTP_CODE=$(echo "$RELEASE_RESP" | tail -1)
    BODY=$(echo "$RELEASE_RESP" | sed '$d')
    RELEASE_ID=$(echo "$BODY" | grep -o '"id": [0-9]*' | head -1 | grep -o '[0-9]*' || true)
